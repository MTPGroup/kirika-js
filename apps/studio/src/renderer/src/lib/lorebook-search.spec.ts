...
import { normalizeSearchText, searchLorebooks, tokenizeSearchQuery } from './lorebook-search'
...
const book = (name: string, description: string, updatedAt = '2026-01-01T00:00:00.000Z'): Book => ({
  name,
  description,
  updatedAt,
})
...
describe('searchLorebooks', () => {
  it('规范化全角字符、大小写和连续空白', () => {
    ...
    expect(tokenizeSearchQuery(' Kirika  kirika 月亮 ')).toEqual(['kirika', '月亮'])
  })
  ...
  it('按照名称完全、前缀、包含和描述命中进行加权排序', () => {
    ...
    expect(searchLorebooks(books, 'kirika')).toEqual([books[3], books[2], books[1], books[0]])
  })
  ...
  it('同分时按更新时间、名称和原始顺序稳定排序', () => {
    ...
    expect(searchLorebooks(books, 'moon')).toEqual([books[1], books[2], books[0]])
