...
async function createProviderClient(input: ProviderConnectionInput) {
  ...
  if (input.apiKey.action === 'retain') {
    const stored = input.providerId
      ? runtime.settings.getProvider(input.providerId)
      : null
  ...
  const apiKey = await runtime.settings.resolveApiKey(
    input.providerId,
    input.apiKey,
  )
...
export const workspaceService: WorkspaceApi = {
  ...
  async getWorkspaceState() {
    return studioRuntime.active
      ? await toWorkspaceState(studioRuntime.active)
      : null
  },
